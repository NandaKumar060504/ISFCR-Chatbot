...
class RAFTDatasetPack(BaseLlamaPack):
    ...
    def __init__(
        ...
        self.embed_model = HuggingFaceEmbedding() if embed_model is None else embed_model

    def strip_str(self, s) -> str:
        """
        Helper function for helping format strings returned by mistralai/Mistral-7B-Instruct-v0.1.
        """
    ...
    def encode_question_gen(self, question, chunk) -> List[str]:
        ...
        return [
            ...
            ChatPromptTemplate(role="user", content=prompt),
        ]

    def generate_label(self, question, context) -> str:
        """
        Generates the label / answer to `question` using `context` and mistralai/Mistral-7B-Instruct-v0.1.
        """
        question_messages = self.encode_question_gen(question, context)
        response = self.llm.ChatOllama(question_messages)
        return str(response)
    ...
    def generate_instructions_gen(self, chunk, x=5) -> List[str]:
        ...
        messages = [
            ...
            ChatPromptTemplate(role="user", content=str(chunk)),
        ]

        queries = str(self.llm.ChatOllama(messages)).split("\n")
...
def main():
    ...
    var = RAFTDatasetPack(file_path='C:\\Users\\Aiml cse\\Desktop\\ISFCR_Codes\\pdfs\\Research_Vol_2021.pdf')
    result=var.run()
    print(result)
